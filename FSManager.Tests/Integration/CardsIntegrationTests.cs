...
END; $$;"
    ...
    [Fact]
    public async Task ShouldFetchRelation() {
        // Arrange
        var client = _factory.CreateClient();
        var key1 = (await CreateCard(client, "key1")).Key;
        var key2 = (await CreateCard(client, "key2")).Key;
        var type = CardRelationType.GUPPY_ITEM;
        await CreateRelation(client, key1, key2, type);

        // Act
        var result1 = await client.GetAsync($"/api/v1/Cards/Relation/{key1}/{key2}");
        var result2 = await client.GetAsync($"/api/v1/Cards/Relation/{key2}/{key1}");

        // Assert
        result1.Should().BeSuccessful();
        result2.Should().BeSuccessful();
        result1.Content.Should().BeEquivalentTo(result2.Content);
        var data = await result1.Content.ReadFromJsonAsync<GetCardRelation>();
        data.Should().NotBeNull();
        data!.RelationType.Should().Be(type);
        data!.RelatedTo.Key.Should().Be(key1);
        data!.RelatedCard.Key.Should().Be(key2);
    }

    [Fact]
    public async Task ShouldNotFetchRelation_CardDoesntExist1() {
        // Arrange
        var client = _factory.CreateClient();
        var key1 = (await CreateCard(client, "key1")).Key;
        var key2 = "key2";
        await CreateRelation(client, key1, key2, CardRelationType.GUPPY_ITEM, false);
        ...
        // Assert
        result1.Should().HaveClientError();
        result2.Should().HaveClientError();
    }

    [Fact]
    public async Task ShouldNotFetchRelation_CardDoesntExist2() {
        // Arrange
        var client = _factory.CreateClient();
        var key1 = "key1";
        var key2 = (await CreateCard(client, "key2")).Key;
    ...
    [Fact]
    public async Task ShouldNotFetchRelation_RelationDoesntExist() {
        // Arrange
        var client = _factory.CreateClient();
        var key1 = (await CreateCard(client, "key1")).Key;
        var key2 = (await CreateCard(client, "key2")).Key;
